...
This chapter presents the detailed design of the proposed system. It explains the internal
organization of modules, workflow of system operations, database structure, interface design,
and detailed interaction between components.

Unlike the High-Level Design chapter, which focuses on the overall architecture and
conceptual organization of the system, this chapter focuses on the internal structural and
functional design of the system components.

 \section[Structure Chart]{Structure Chart}
This section presents the structural decomposition and organization of the system into
...
\subsubsection{Friction Sentry Module}
\textbf{A. Purpose} \\
...
\textbf{B. Contribution to Overall System} \\
...
\subsubsection{Networked Digital Twin Module}
\textbf{A. Purpose} \\
...
\subsubsection{Logistics Optimizer Module}
\textbf{A. Purpose} \\
...
\subsubsection{Interactive Dashboard Module}
\textbf{A. Purpose} \\
...
\begin{figure}[H]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.25\textwidth]{Figures/Module hierarchy.png}}
...
\subsection[Module Interaction Flow]{Module Interaction Flow}
...
\begin{figure}[H]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.25\textwidth]{Figures/Interaction flow diagram.png}}
...
\textbf{Purpose:} Defines physical facilities and processing nodes within the logistics network.

\begin{table}[H]
\centering
\begin{tabularx}{\textwidth}{|l|l|>{\hsize=0.8\hsize}X|>{\hsize=1.2\hsize}X|}
\hline
...
\textbf{Purpose:} Represents shipping lanes connecting physical nodes in the graph.
...
\textbf{Purpose:} Tracks real-time stock parameters for destination nodes.
...
\begin{itemize}
    ...
    \begin{enumerate}
        ...
        \item \textbf{Right Panel (Control \& Telemetry Panel):} Hosts the strategy configuration controls, optimization triggers, and dynamic performance cards (e.g., Transit Time Delta, Cost Delta, Stockout Alarms, Executive Summaries).
    \end{enumerate}
...
\begin{lstlisting}[language=json]
{
  "status": "active",
  "location": "red_sea",
  "message_type": "Houthi Attack Alert",
  "severity_multiplier": 1.60,
  "analysis": "Drone attack alert in the Red Sea corridor directly impacting ocean transit lanes."
}
\end{lstlisting}
...
\begin{itemize}
    ...
    The system executes a sequence of logical operations starting from input validation, injecting shock multipliers into the graph model, computing optimized paths via Dijkstra's algorithm, orchestrating the CrewAI reasoning loop, and performing final inventory health checks, as illustrated in Figure~\ref{fig:internal_flow}.

    \begin{figure}[H]
        \centering
        \makebox[\textwidth][c]{\includegraphics[width=1.25\textwidth]{Figures/Internal flow.png}}
        \caption{Internal Processing Stages and Logical Workflow}
    ...
    \end{itemize}
    \item \textbf{Module Coordination \& Sequence:} \\
    Execution follows a strict sequential pipeline:
    \[ \text{Friction Ingestion} \rightarrow \text{Graph Shock Calculation} \rightarrow \text{Dijkstra Routing} \rightarrow \text{Agentic Analysis \& Stock Check} \rightarrow \text{Payload Consolidation} \]
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Graceful API Degradation:} Under API errors, the UI replaces detailed summaries with predefined, simplified alert messages while maintaining graph rendering.
    \end{itemize}
\end{itemize}
